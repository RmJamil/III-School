...
const isSubmitted = (assignmentId) =>
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <div className='p-6 border rounded-2xl flex justify-between'>
       <div>
         <div><span className='font-bold'>Class Name : </span><span>{enrolledClass.title}</span></div>
     <div> <span className='font-bold'>Instructor:</span> <span>{enrolledClass.name}</span> </div>
       </div>
       <div>
        <button   onClick={() => setShowModal(true)} className='btn flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600'>   
  ...
  {isPageLoading ? (
    ...
    <button className="btn btn-sm btn-disabled ">Already Submitted</button>
  ) : (
    <button
          ...
          <div className=" p-6 rounded-lg w-full max-w-md shadow-lg">
            ...
            <div className="flex justify-end gap-2">
              <button
                ...
                className=" btn px-4 py-2 bg-gray-300 rounded hover:bg-red-500 hover:text-white border-none"
              >
                Cancel
              </button>
              <button
                onClick={handleSend}
                className="btn px-4 py-2 border border-green-500 rounded hover:bg-green-500 hover:text-white"
              >
                Send
              </button>
            </div>
